...
/** A quiet, distant street; one small panorama avoids an exterior room full of geometry. */
export function doorwayNightTexture() {
    return canvasTexture(1024, 512, ctx => {
        const sky = ctx.createLinearGradient(0, 0, 0, 512);
        sky.addColorStop(0, '#11202b');
        sky.addColorStop(.6, '#263c45');
        sky.addColorStop(1, '#182729');
        ctx.fillStyle = sky;
        ctx.fillRect(0, 0, 1024, 512);
        let seed = 47;
        const random = () => { seed = (seed * 16807) % 2147483647; return seed / 2147483647; };
        ctx.filter = 'blur(2px)';
        for (let x = -20; x < 1024; x += 65) {
            const top = 120 + random() * 140;
            ctx.fillStyle = random() > .5 ? '#16282f' : '#1b3037';
            ctx.fillRect(x, top, 61, 350);
            for (let row = 0; row < 6; row++) for (let col = 0; col < 3; col++) {
                if (random() > .28) continue;
                ctx.fillStyle = random() > .45 ? '#b8a47b70' : '#87abb24d';
                ctx.fillRect(x + 9 + col * 16, top + 15 + row * 24, 5, 8);
            }
        }
        // Diffuse street lamps and reflections suggest wet pavement, without hard corners.
        for (const x of [130, 445, 730, 960]) {
            const glow = ctx.createRadialGradient(x, 365, 1, x, 365, 45);
            glow.addColorStop(0, '#c9ad7640');
            glow.addColorStop(1, '#c9ad7600');
            ctx.fillStyle = glow;
            ctx.fillRect(x - 45, 320, 90, 90);
            ctx.fillStyle = '#c9ad7630';
            ctx.fillRect(x - 2, 361, 4, 7);
        }
        ctx.filter = 'none';
        const mist = ctx.createLinearGradient(0, 310, 0, 512);
        mist.addColorStop(0, '#1b2d3300');
        mist.addColorStop(1, '#223436');
        ctx.fillStyle = mist;
        ctx.fillRect(0, 310, 1024, 202);
    });
}
